<template>
    <div class="fo-kannada"> <br>

      <v-btn primary color="grey" style="margin:auto; display:block;border:2px solid black;width:100%" @click="help_guide"> ಮಾರ್ಗದರ್ಶನ ಕಾಗಿ ಇಲ್ಲಿ click ಮಾಡಿ  </v-btn>

      <h2 class="text-md-center"> ಮೂಲ ಮಾಹಿತಿ </h2>
      ...
      <div class="input-section" v-intro-step="1" v-intro="'Welcome to Fertilizer Optimizer. This is a native application to suggest the most optimal fertilizer combination for an input crop-irrigation type combination and soil conditions. Click Next to proceed further with the Getting Started Guide.'">
        ...
        <v-layout row  v-intro-step="2" v-intro="'Select the state, district, taluka and Panchayat of the farmer from the dropdown'">
          <v-flex md6>
            <v-flex md10>
              <v-select :items="state_select" :rules="[() => !!selected_state || 'This field is required']" v-model="selected_state" label="ರಾಜ್ಯ" required></v-select>
              ರಾಜ್ಯ: <span  class="highlight-input">  {{selected_state}} </span> <br><br>
            </v-flex>
          </v-flex>

          <v-flex md6>
            <v-flex md10>
              <v-select :items="district_select" :rules="[() => !!selected_district || 'This field is required']" v-model="selected_district" label="ಜಿಲ್ಲೆ" required></v-select>
              ಜಿಲ್ಲೆ: <span  class="highlight-input">  {{selected_district}} </span> <br><br>
            </v-flex>
          </v-flex>
          </v-layout>
          ...
          <v-flex md6>
            <v-flex md10>
              <v-select :items="taluka_select" :rules="[() => !!selected_taluka || 'This field is required']" v-model="selected_taluka" label="ತಾಲ್ಲೂಕು" required></v-select>
              ತಾಲ್ಲೂಕು: <span  class="highlight-input">  {{selected_taluka}} </span> <br><br>
            </v-flex>
          </v-flex>

          <v-flex md6>
            <v-flex md10>
              <v-select :items="panchayat_select" :rules="[() => !!selected_panchayat || 'This field is required']" v-model="selected_panchayat" label="ಗ್ರಾಮ ಪಂಚಾಯತ್" required></v-select>
              ಗ್ರಾಮ ಪಂಚಾಯತ್: <span  class="highlight-input">  {{panchayat_code}}  - {{panchayat_name}}</span> <br><br>
            </v-flex>
          </v-flex>
        </v-layout>
          ...
          <v-layout row>
              <v-flex md6>
                <div class="select-crop" v-intro-step="3" v-intro="'Select the crop name from this dropdown.'">
                  <v-flex md8>
                    <v-select :items="crop_select" :rules="[() => !!selected_crop || 'This field is required']" v-model="selected_crop" label="ಬೆಳೆ" required></v-select>
                    ಬೆಳೆ: <span  class="highlight-input">  {{selected_crop}} </span>
                  </v-flex>
                </div>
              </v-flex>
              ...
              <v-flex md6>
                <div class="select-irrigation-type" v-intro-step="4" v-intro="'Select whether the crop is being irrigated or rainfed using this dropdown.'">
                  <v-flex md8>
                    <v-select :items="irrigation_type_select" :rules="[() => !!selected_irrigation_type || 'This field is required']" v-model="selected_irrigation_type" label="ನೀರಾವರಿ ವಿಧ" required></v-select>
                    ನೀರಾವರಿ ವಿಧ: <span  class="highlight-input">  {{selected_irrigation_type}} </span>
                  </v-flex>
                </div>
              </v-flex>
          </v-layout>
          ...
          <v-layout row v-intro-step="5" v-intro="'Select the Variety of the crop from the dropdown. If the variety name is not available in dropdown, choose Other in dropdown and enter the variety name as input text.'">
              ...
              <v-flex md6>
                <v-flex md8>
                  <v-select :items="variety_list" :rules="[() => !!selected_variety || 'This field is required']" v-model="selected_variety" label="ವಿಧ" required></v-select>
                  ವಿಧ: <span  class="highlight-input"> {{variety_text}} </span> </p>
              ...
              <v-flex md6>
                <v-flex md8>
                  <div id="enable-text-box" v-if="selected_variety === 'Others'">
                    <v-text-field v-model.number="variety_text" label="ವಿವಿಧ" v-on:keyup="update_variety()"></v-text-field>
                  </div>
                  <br> <br>
                </v-flex>
              </v-flex>
          ...
          <v-layout row>
              <v-flex md6>
                <div class="farmer-ID" v-intro-step="6" v-intro="'Enter the farmer ID# for whom this query is being carried out'">
                  <v-flex md8>
                  ರೈತ ID: <br> <input type="text" v-model="taluka_code" size="1" readonly> - <input type="text" v-model="panchayat_code" size="1" readonly> -
                    ...
                    ರೈತ ID: <span  class="highlight-input">  {{farmer_ID}} </span>
                  </v-flex>
                </div>
              </v-flex>
              ...
              <v-flex md6>
                ...
                ಎಕರೆ ಪ್ರದೇಶದಲ್ಲಿ: <input id="area_acre" type="number" placeholder="0" v-model="area_acre" v-on:keyup="fertilizer_calc()" style="width:300px"/> <br>
                <span  class="highlight-input"> {{area_acre}} acre(s) </span>
                </v-flex>
              </v-flex>
          </v-layout> <br>
          ...
          <v-layout row>
              <v-flex md12  v-intro-step="10" v-intro="'Click this button to compute N,P,K deficit and optimized fertilizer '">
                <div class="text-md-center">
                  <a href="#n-p-k-calc">
                    <v-btn round justify-center color="primary" @click.native="process_inputs()" :disabled="disable_button_inputs"><v-icon> pie_chart </v-icon> N-P-K ಅಂಕಿ ಅಂಶವನ್ನು ಸೃಷ್ಟಿಸುತ್ತದೆು </v-btn>
                  </a>
                </div>
              </v-flex>
          </v-layout>
      ...
      <div id="n-p-k-calc" class="n-p-k-calc-section"> <br>
          <p> ಪ್ರತಿ ಎಕರೆಗೆ ಆಯ್ಕೆ ಮಾಡಿದ ಬೆಳೆ / ನೀರಾವರಿ ವಿಧಕ್ಕೆ ರಸಗೊಬ್ಬರ ಅಗತ್ಯ
          ...
          <v-layout row wrap >
            <v-flex md6>
              ...
              ಬೆಳೆ / ನೀರಾವರಿ ಮಾದರಿಯ ಒಟ್ಟು ರಸಗೊಬ್ಬರವು ಆಯ್ಕೆ ಮಾಡಿದೆ <span class="highlight-input"> {{area_acre}} acre(s) </span> <br>
            ...
            <v-flex md6>
              <v-card style="background-color:#bcbec0;color:#B11117">
                N, P, K ನಿಮ್ಮ ಮಣ್ಣಿನ ಪರೀಕ್ಷಾ ಫಲಿತಾಂಶದ ಆಧಾರದ ಮೇಲೆ ನಿಮ್ಮ ಮಣ್ಣಿನ ಫಲೀಕರಣ ಮತ್ತಷ್ಟು ಬೇಕಾಗುತ್ತದೆ: <br>
      ...
      <div class="text-md-center">
          <v-btn round justify-center :to="{name: 'FertilizerOptimizerKannada'}" :disabled="disable_button_next"> ಮುಂದೆ </v-btn>
      </div> <br>
    </div>
</template>
